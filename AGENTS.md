...
- `/docs/architecture/` — durable system design and architectural decisions.
- `/docs/research/` — reusable investigations, references, and findings.
- `/docs/issues/<number>/` — issue-scoped research, designs, decisions, and acceptance plans.

Link each issue document to its GitHub issue and link the issue back to its documents. After implementation, reflect lasting conclusions in `/docs/architecture/` or `/docs/principles/`; retain the issue documents as historical rationale.
...
- **Research before designing:** For architectural or external integrations, inspect first-party documentation and relevant reference implementations. Record issue-specific findings and unresolved decisions under `/docs/issues/<number>/`; use `/docs/research/` only when the material is reusable beyond the issue.
- **Get design approval before implementing:** After research, present the findings, recommended direction, meaningful alternatives and tradeoffs, unresolved decisions, and proposed acceptance evidence to the user as a proposal. Stop and obtain the user's explicit approval or requested revisions before editing production code or treating the direction as resolved. The user must have enough information to understand the consequential choices; a request to start work is not itself approval of a design that has not yet been discussed.
- **Define acceptance evidence:** After design approval, update the issue from the approved design before coding, including production-path tests, evaluations, performance checks, and human verification where applicable.
